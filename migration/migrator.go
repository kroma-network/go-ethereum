...
func (m *StateMigrator) Start() {
	...
	go func() {
		if m.migratedRef.Root() == (common.Hash{}) {
			...
			err = m.ValidateMigratedState(m.ctx, m.migratedRef.Root(), safeHead.Root)
			if err != nil {
...
func (m *StateMigrator) FinalizeTransition(transitionBlock types.Block) {
	...
	go func() {
		...
		if err := m.ValidateMigratedState(context.Background(), m.migratedRef.Root(), zkBlock.Root()); err != nil {
			panic(err)
		}
