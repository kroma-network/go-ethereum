...
type proofResult struct {
	...
	tr       trie.LowLevelTrie // The trie, in case the trie was resolved by the prover (may be nil)
}
...
func (dl *diskLayer) proveRange(ctx *generatorContext, trieId *trie.ID, prefix []byte, kind string, origin []byte, max int, valueConvertFn func([]byte) ([]byte, error)) (*proofResult, error) {
	...
	if origin == nil && !diskMore {
		if dl.triedb.Zktrie {
			zktrie, _ := trie.NewZkTrie(common.Hash{}, trie.NewZktrieDatabase(memorydb.New()))
			for i, key := range keys {
				zktrie.TryUpdate(key, vals[i])
			}
			if gotRoot := zktrie.Hash(); gotRoot != root {
				return &proofResult{
					keys:     keys,
					vals:     vals,
	...
	tr, err := dl.OpenLowLevelTrie(trieId)
	if err != nil {
...
func generateAccounts(ctx *generatorContext, dl *diskLayer, accMarker []byte) error {
	onAccount := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if (dl.triedb.Zktrie && bytes.Equal(acc.Root[:], zkt.HashZero[:])) ||
			acc.Root == emptyRoot {
			ctx.removeStorageAt(account)
		} else {
