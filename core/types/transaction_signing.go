...
func MakeSigner(config *params.ChainConfig, blockNumber *big.Int, blockTime uint64) Signer {
	var signer Signer
	switch {
	case config.IsPrague(blockNumber, blockTime) && !config.IsKroma():
		signer = NewPragueSigner(config.ChainID)
	case config.IsCancun(blockNumber, blockTime) && !config.IsKroma():
...
func LatestSigner(config *params.ChainConfig) Signer {
	...
	if config.ChainID != nil {
		switch {
		case config.PragueTime != nil && !config.IsKroma():
			signer = NewPragueSigner(config.ChainID)
		case config.CancunTime != nil && !config.IsKroma():
			signer = NewCancunSigner(config.ChainID)
...
func (s londonSigner) Sender(tx *Transaction) (common.Address, error) {
	if tx.Type() == DepositTxType {
		...
		case *kromaDepositTxWithNonce:
			return tx.inner.(*kromaDepositTxWithNonce).From, nil
		}
	}
