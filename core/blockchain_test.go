...
func TestDeleteThenCreate(t *testing.T) {
	// [kroma unsupported]
	t.Skip("selfdestruct unsupported")
	var (
		engine      = ethash.NewFaker()
		key, _      = crypto.HexToECDSA("b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291")
		address     = crypto.PubkeyToAddress(key.PublicKey)
...
func TestEIP3651(t *testing.T) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, engine, vm.Config{Tracer: vm.NewMarkdownLogger(&vm.LogConfig{}, os.Stderr)}, nil, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func TestPoseidonCodeHash(t *testing.T) {
	...
	chain, receipts := GenerateChain(&config, genesis, engine, db, 1, func(i int, gen *BlockGen) {
	...
	chain, receipts = GenerateChain(&config, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), engine, db, 1, func(i int, gen *BlockGen) {
...
func TestTransactionCountLimit(t *testing.T) {
	...
	for _, txCountLimit := range []int{0, 1} {
		testFunc(txCountLimit)
	}
}
...
func TestBlockPayloadSizeLimit(t *testing.T) {
	...
	for _, blockLimit := range []int{0, 150} {
		testFunc(blockLimit)
	}
}
