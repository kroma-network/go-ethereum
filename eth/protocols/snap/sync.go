...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	for i, hash := range res.hashes {
		...
		slim := types.SlimAccountBytes(*res.accounts[i], false)
		rawdb.WriteAccountSnapshot(batch, hash, slim)

		// If the task is complete, drop it into the stack trie to generate
		// account trie nodes for it
		if !task.needHeal[i] {
			full, err := types.FullAccountBytes(slim, false) // TODO(karalabe): Slim parsing can be omitted
			if err != nil {
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	if len(paths) == 1 {
		account, err := types.NewStateAccount(value, false)
		if err != nil {
			return nil // Returning the error here would drop the remote peer
		}
		blob := types.SlimAccountBytes(*account, false)
