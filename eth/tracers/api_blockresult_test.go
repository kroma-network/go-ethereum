// This is taken from https://github.com/scroll-tech/go-ethereum/blob/staging/eth/tracers/api_blockresult_test.go
...
func checkStructLogs(t *testing.T, expect []*txTraceResult, actual []*types.ExecutionResult) {
	...
	for i, val := range expect {
		...
		// NOTE: I don't know exactly since when traceTx() always returns json.RawMessage.
		// But since then, val.Result should be unmarshaled to types.ExecutionResult.
		var trace1 types.ExecutionResult
