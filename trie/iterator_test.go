...
func TestMerkleTreeIterator(t *testing.T) {
	...
	t.Run("startKey", func(t *testing.T) {
		...
		// I intentionally did not create it in a test template format in order to debug and run it one by one.
		// tests are classified into 2 types
		// fixed index: test name means tree leaf node index. Start iteration from the index and verify the number of leaves
		// range index: test name contains ~. It means start to end and randomly selects an index between these to start iteration. Verify the number of leaves
		// leaf hit annotation means the exact index of the leaf node
		// The test names at the bottom were created with the makeTreeInput function, and since the random seed is fixed, the test will fail if changed.
