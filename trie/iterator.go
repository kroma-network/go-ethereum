...
type merkleTreeIterator struct {
	...
	// stack length should never be 0 if there are leaves to visit.
...
func (it *merkleTreeIterator) seek(path zk.TreePath) {
	...
	if _, ok := it.stack[len(it.stack)-1].(*merkleTreeIteratorParentNode); ok && it.err == nil {
		it.nextLeaf()
	}

	if leaf, ok := it.stack[len(it.stack)-1].(*merkleTreeIteratorLeafNode); ok && it.err == nil {
