...
func initGenesis(ctx *cli.Context) error {
	...
	for _, name := range []string{"chaindata", "lightchaindata"} {
		...
		triedb := utils.MakeTrieDatabase(ctx, chaindb, ctx.Bool(utils.CachePreimagesFlag.Name), false, genesis.IsVerkle(), genesis.Config.Zktrie)
		defer triedb.Close()
...
func dump(ctx *cli.Context) error {
	...
	triedb := utils.MakeTrieDatabase(ctx, db, true, true, false, cfg.Eth.Genesis.Config.Zktrie) // always enable preimage lookup
	defer triedb.Close()
